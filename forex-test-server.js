      ...
      try {
        ...
        chart.onUpdate(() => {
          if (chart.periods && chart.periods[0] && isAuthenticated) {
            ...
            // Firebase mein data save karo (only if all values are valid)
            if (data.price && data.open && data.high && data.low) {
              db.ref(`prices/${pairName}`).set(data).catch(err => console.log(`Firebase error for ${pairName}:`, err.message));
            }
